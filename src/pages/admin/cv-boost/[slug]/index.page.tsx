...
export default withAuth(AdminDetailCVBoost, 'admin');
function AdminDetailCVBoost() {
  ...
  queryFn: async () => {
    const res = await axios.get<ApiReturn<CVDetail>>(`/booster/cv/${slug}`);
    return res.data;
  },
});

  const methods = useForm<PostCVReview>({
    mode: 'onTouched',
  });
  const { handleSubmit } = methods;

  const { mutate: PostReviewMutate, isPending: PostReviewLoading } =
    useMutationToast<void, PostCVReview>(
      useMutation({
        mutationFn: async (data) => {
          await api.post(`/booster/cv/review/${slug}`, data);
        },
        onSuccess: () => {
  ...
  const onSubmit: SubmitHandler<PostCVReview> = (data) => {
